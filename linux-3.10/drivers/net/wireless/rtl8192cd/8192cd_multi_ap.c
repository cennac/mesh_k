...
void rtk_multi_ap_sta_rssi_trigger(struct rtl8192cd_priv *priv, struct stat_info *pstat){
	...
	rssi_threshold = priv->pmib->multi_ap.multiap_rssi_threshold;

	//If the latest station rssi has crossed the rssi threshold with respect to last measured
	if(((pstat->last_rssi < rssi_threshold) && (current_rssi > rssi_threshold)) ||
		((pstat->last_rssi > rssi_threshold) && (current_rssi < rssi_threshold))){
