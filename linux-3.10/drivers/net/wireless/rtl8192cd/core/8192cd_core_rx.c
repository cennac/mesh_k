...
#ifdef A4_STA
	                if(GET_BR_PORT(priv->dev) && priv->pshare->rf_ft_var.a4_enable == 2 && fdb_find_for_driver(GET_BR_PORT(priv->dev)->br, da) == NULL) {
...
#ifdef A4_STA
                if(GET_BR_PORT(priv->dev) && priv->pshare->rf_ft_var.a4_enable == 2 && fdb_find_for_driver(GET_BR_PORT(priv->dev)->br, pfrinfo->da) == NULL) {
                    pnewskb = skb_copy(pskb, GFP_ATOMIC);
                    if (pnewskb) {
#ifdef GBWC
                        if (GBWC_forward_check(priv, pnewskb, pstat)) {
