...
//step2
	...
	if (gs_type == GSK3) {
		...
		attenu = (u8)odm_get_rf_reg(dm, (enum rf_path)path,
			    0x63, 0xf000);
		if (gain_db > -20) {
			attenu = attenu + 2;
		...
		odm_set_bb_reg(dm, 0x1bb8, BIT(20), 0x0);
		odm_set_rf_reg(dm, (enum rf_path)path, 0x63, 0xf000, attenu);
		rf63 = odm_get_rf_reg(dm, (enum rf_path)path, 0x63, 0xfffff);
		RF_DBG(dm, DBG_RF_DPK, "[DPK][GSK3] rf63 = 0x%x\n", rf63);
	}
	...
	if (gs_type == GSK2) {
		...
		odm_set_rf_reg(dm, (enum rf_path)path, 0x63, 0xf000, attenu);
		rf63 = odm_get_rf_reg(dm, (enum rf_path)path, 0x63, 0xfffff);
		RF_DBG(dm, DBG_RF_DPK, "[DPK][GSK2] rf63 = 0x%x\n", rf63);
	}
//step3
	if (gs_type == GSK1) {
		...
		rf63 = odm_get_rf_reg(dm, (enum rf_path)path, 0x63, 0xfffff);
		RF_DBG(dm, DBG_RF_DPK, "[DPK][GSK1] rf63 = 0x%x\n", rf63);
	}
	return fail;
}
...
void do_dpk_8195b(void *dm_void)
{
	...
	if (dm->ext_pa) {
		...
		RF_DBG(dm, DBG_RF_DPK,
		       "[DPK]***DPK Start (Ver: %s), Cut: %d, Package: %d***\n",
