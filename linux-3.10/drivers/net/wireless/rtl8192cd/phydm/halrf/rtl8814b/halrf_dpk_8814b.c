...
*/
	if (gs_type == GSK3) {
		...
		odm_set_rf_reg(dm, (enum rf_path)path, RF_0x00, 0x0003e0, tmp_rxbb);
		rf0 = odm_get_rf_reg(dm, (enum rf_path)path, RF_0x00, 0xfffff);
...
boolean _do_dpk_8814b(void *dm_void)
{
	...
	u32 bp_bb_reg[DPK_BB_NUM_8814B] = {0x1c38, 0x1c68, 0x1d60, 0x180c, 0x410c,
				       0x520c, 0x530c, 0x1d70, 0x1a00, 0x824,
				       0x1a14, 0x183c, 0x413c, 0x523c, 0x533c,
				       0x1840, 0x1844, 0x4140, 0x4144, 0x5240,
				       0x5244, 0x5340, 0x5344};
	u32 bp_rf_reg[DPK_RF_NUM_8814B] = {0x88, 0x8c, 0xde};
	
	u8 path = 0x0, retry_cnt = 0, tx_agc_search, k, dpk_step = 0x0;
	u8 gsk1_retry_count = 0x0, gsk2_retry_count = 0x0, gl_retry_count = 0x0;
	u16 i;
	...
	for (path = 0; path < SS_8814B; path++) {
		...
		case 2:	
			...
			ODM_delay_ms(1);
			gonext = false;
			gsk2_retry_count = 0x0;			
			while (1) {
				KFAIL = _dpk_pas_agc_8814b(dm, path, GSK2);
				if (KFAIL && (gsk2_retry_count < 1))
					gsk2_retry_count++;
				else if (KFAIL) {
					if (gl_retry_count < 2)
						dpk_step = 1;
