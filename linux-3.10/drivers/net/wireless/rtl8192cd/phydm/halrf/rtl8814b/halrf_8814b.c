...
void _halrf_trigger_tssi_8814b(
	void *dm_void,
	u8 rf_path)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
...
void _halrf_get_tssi_result_8814b(
	void *dm_void,
	u8 rf_path)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	...
	u8 rf_path)
{
	...
	if (!tssi->thermal_trigger) {
		_halrf_trigger_tssi_8814b(dm, rf_path);
	} else {
		_halrf_get_tssi_result_8814b(dm, rf_path);
	}
	...
	void *dm_void, u8 direct_do)
{
	...
	for (i = RF_PATH_A; i < MAX_PATH_NUM_8814B; i++) {
		...
		if (channel >= 1 && channel <= 14) {
			...
			odm_set_bb_reg(dm, R_0x1bcc, 0x000000ff, 0x3f);
			...
			odm_set_bb_reg(dm, R_0x1b00, 0x00000006, i);
			odm_set_bb_reg(dm, R_0x1bcc, 0x000000ff, 0x0);
			odm_set_bb_reg(dm, R_0x1ca4, 0x00000001, 0x0);
