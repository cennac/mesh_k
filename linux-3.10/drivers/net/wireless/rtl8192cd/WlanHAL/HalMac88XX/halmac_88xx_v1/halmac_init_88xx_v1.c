...
#define ANAPAR_MAC0_CFG_A_CUT	0x1A
#define ANAPAR_MAC0_CFG		0x19
...
init_wmac_cfg_88xx_v1(struct halmac_adapter *adapter)
{
	...
	union halmac_wlan_addr m_addr = {{0xFF, 0xFF, 0xFF,
					 0xFF, 0xFF, 0xFF},};

	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);

	cfg_multicast_addr_88xx_v1(adapter, &m_addr);
...
init_analog_ip_cfg_88xx_v1(struct halmac_adapter *adapter)
{
	...
	if (adapter->chip_ver == HALMAC_CHIP_VER_A_CUT) {
		HALMAC_REG_W8(REG_ANAPAR_MAC_0 + 2, ANAPAR_MAC0_CFG_A_CUT);
		HALMAC_REG_W32(REG_ANAPAR_MAC_1, ANAPAR_MAC1_CFG_A_CUT);
	} else {
		HALMAC_REG_W8(REG_ANAPAR_MAC_0 + 2, ANAPAR_MAC0_CFG);
