...
//8814B ADDRCAM Init for each interface
	...
	if(priv->pmib->ethBrExtInfo.macclone_enable && IS_HAL_CHIP(priv) && IS_VXD_INTERFACE(priv)) 
	{
		...
		macAddr[0] |= BIT1;
		memcpy(priv->dev->dev_addr, macAddr, MACADDRLEN);
...
//	if (IS_VXD_INTERFACE(priv) && !GET_ROOT_PRIV(priv)->pmib->miscEntry.func_off) 
	...
	if(IS_VAP_INTERFACE(priv) || IS_VXD_INTERFACE(priv))
		priv->pshare->rf_ft_var.a4_enable = 1;
#endif
#endif
...
#if defined(CONFIG_WLAN_HAL_8192FE)
	if (GET_CHIP_VER(priv)==VERSION_8192F) {
#if defined(CONFIG_SLOT_0_RFE_TYPE_9)
		priv->pmib->dot11RFEntry.rfe_type = 9;
#else		
		priv->pmib->dot11RFEntry.rfe_type = 3;
#endif
...
#if defined(CONFIG_WLAN_HAL_8814BE)
            //yllin8814B
            //config address不能直接assign, 不知道進kernel後address變多少
#endif
#endif
